...
- 顶部“新建项目”按钮：创建新项目并进入编辑器。
- 选择项目进入编辑器，同时打开右侧项目设置抽屉。
...
- 顶部工具条：
  - 保存、发布入口。
- 右侧项目设置抽屉（可折叠）：
  - 基本信息：项目名称、简介、封面 URL、标签。
  - 发布状态：草稿 / 已发布。
  - 定价：文本阅读、AI 语音积分。
  - 权限：允许复制、允许下载、水印保护。
  - 保存设置按钮。
- 文本编辑：
  - 单列 Notion 风格编辑区（所见即所得的文本输入）。
  - 不显示 Markdown 源码与预览分栏。
  - 支持标题/正文/列表/图片/附件等区块化编辑，图片可通过 URL 直接预览。
  - 块左侧提供“+”菜单，悬浮触发展开，可插入标题/列表/图片/附件等（原型交互）。
  - 底部提示“/”唤起模块菜单（原型占位）。
- 非文本文件：
  - 提示暂不支持编辑/预览。
